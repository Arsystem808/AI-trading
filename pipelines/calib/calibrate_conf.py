...
def main() -> None:
    ...
    # Суррогатная «сырая» уверенность для подбора b
    ...
    # Обновляем калибровку для M7 (и, при желании, для Octopus тем же сдвигом)
    base.setdefault("M7", {}).setdefault("conf", {"method": "sigmoid", "params": {"a": 1.0, "b": 0.0}})
    base["M7"]["conf"]["params"]["b"] = float(b_best)

    if "Octopus" in base:
        base["Octopus"].setdefault("conf", {"method": "sigmoid", "params": {"a": 1.2, "b": -0.10}})
        base["Octopus"]["conf"]["params"]["b"] = float(b_best)
