...
def _get_market_session(hour: int, minute: int) -> str:
“”“Определение торговой сессии по времени”””
...
if 570 <= time_minutes <= 585:  # 9:30-9:45
    ...
    return "entry_prime"
elif 720 <= time_minutes <= 810:  # 12:00-13:30
    return "entry_lunch"
elif 840 <= time_minutes <= 930:  # 14:00-15:30
    return "scalp_window"
else:
    return "inactive"
```

def _classify_universal_instrument(ticker: str, price: float) -> dict:
“”“Классификация инструмента для универсальной стратегии”””

```
# Крипто ETF - высокая волатильность
if '-X' in ticker or ticker in ['BTC', 'ETH']:
    return {'type': 'crypto', 'target_profit': 0.089, 'risk_tolerance': 0.05}

# Левереджированные ETF  
if any(x in ticker for x in ['SOXL', 'SOXS', 'QID', 'KLAC', 'NFLX']):
    return {'type': 'leveraged_etf', 'target_profit': 0.074, 'risk_tolerance': 0.05}

# Рост-акции
if ticker in ['NVDA', 'TSLA', 'AAPL', 'META'] or price > 200:
    return {'type': 'growth', 'target_profit': 0.068, 'risk_tolerance': 0.03}

# Шорт-специализация 
if ticker in ['MSFT']:
    return {'type': 'short_bias', 'target_profit': 0.053, 'risk_tolerance': 0.03}

# Value акции по умолчанию
return {'type': 'value', 'target_profit': 0.055, 'risk_tolerance': 0.03}
```

def _calculate_dynamic_levels(df: pd.DataFrame, current_price: float) -> dict:
“”“Динамические уровни без индикаторов - только цена”””

```
# Берем данные за последние 5 дней для динамических уровней
recent = df.tail(5)
today = df.tail(1).iloc[0]

high_5d = float(recent["high"].max())
low_5d = float(recent["low"].min()) 
high_today = float(today["high"])
low_today = float(today["low"])

# Диапазоны для определения качества сетапа
range_5d = high_5d - low_5d
range_today = high_today - low_today

return {
    # ЛОНГ ЗОНЫ (покупка на откатах)
    'long_zone_1': low_5d * 1.02,        # +2% от 5-дневного минимума
    'long_zone_2': high_5d * 0.93,       # -7% от 5-дневного максимума
    'long_zone_3': high_today * 0.96,    # -4% от максимума дня
    
    # ШОРТ ЗОНЫ (продажа на пиках)  
    'short_zone_1': high_5d * 0.98,      # -2% от 5-дневного максимума
    'short_zone_2': low_5d * 1.07,       # +7% от 5-дневного минимума
    'short_zone_3': low_today * 1.04,    # +4% от минимума дня
...
def _universal_entry_logic(ticker: str, price: float, levels: dict,
volume_ratio: float, session: str) -> tuple:
“”“Основная логика входа универсальной стратегии”””

```
# Временной фильтр - входим только в правильные сессии
if session not in ['entry_prime', 'entry_lunch', 'scalp_window']:
    return "WAIT", "outside_entry_window", 0.45

# Объемный фильтр - нужен повышенный интерес  
if volume_ratio < 1.2:
    return "WAIT", "low_volume", 0.50

instrument = _classify_universal_instrument(ticker, price)

# ЛОНГ УСЛОВИЯ - покупка на откатах
long_triggers = [
    price <= levels['long_zone_1'],  # У 5-дневного минимума
    price <= levels['long_zone_2'],  # Откат от 5-дневного максимума
    price <= levels['long_zone_3']   # Откат от дневного максимума
]

if any(long_triggers) and instrument['type'] != 'short_bias':
    # Дополнительная проверка: не покупаем в свободном падении
    if levels['range_today'] > 0:
        position_in_daily_range = (price - levels['low_today']) / levels['range_today']
        if position_in_daily_range < 0.3:  # В нижних 30% дневного диапазона
            confidence = 0.65 + 0.15 * (1.2 - position_in_daily_range)
            return "BUY", f"{session}_long_dip", min(0.85, confidence)

# ШОРТ УСЛОВИЯ - продажа на силе
short_triggers = [
    price >= levels['short_zone_1'],  # У 5-дневного максимума
    price >= levels['short_zone_2'],  # Рост от 5-дневного минимума
    price >= levels['short_zone_3']   # Рост от дневного минимума
]

if any(short_triggers):
    # Дополнительная проверка: продаем на пиках
    if levels['range_today'] > 0:
        position_in_daily_range = (price - levels['low_today']) / levels['range_today']
        if position_in_daily_range > 0.7:  # В верхних 30% дневного диапазона
            confidence = 0.65 + 0.15 * (position_in_daily_range - 0.7) / 0.3
            return "SHORT", f"{session}_short_strength", min(0.85, confidence)

return "WAIT", "no_clear_setup", 0.52
```

def _universal_position_size(price: float, account_balance: float = 50000) -> int:
“”“Универсальный расчет размера позиции”””
risk_pct = 0.02  # 2% риска на сделку
risk_amount = account_balance * risk_pct
stop_distance = price * 0.05  # 5% стоп-лосс
base_shares = int(risk_amount / stop_distance)

```
# Ограничения по ликвидности
if price >= 1000:
    max_shares = min(base_shares, 20)
elif price >= 500:
    max_shares = min(base_shares, 50)
elif price >= 200:
    max_shares = min(base_shares, 100)  
elif price >= 50:
    max_shares = min(base_shares, 300)
elif price >= 10:
    max_shares = min(base_shares, 500)
else:
    max_shares = min(base_shares, 1000)

return max(max_shares, 10)
```

def analyze_asset_universal(ticker: str, horizon: str = “Краткосрочно”):
“””
Универсальная стратегия на основе анализа 400+ успешных сделок
Основа: ВРЕМЯ + ЦЕНА + ОБЪЕМ (без индикаторов)
“””

```
if horizon != "Краткосрочно":
    # Для других горизонтов используем основную стратегию
    return analyze_asset(ticker, horizon)

cli = PolygonClient()

# Данные за последние 30 дней (достаточно для краткосрочной стратегии)
df = cli.daily_ohlc(ticker, days=30)
price = cli.last_trade_price(ticker)

# Получаем текущее время (в продакшене нужно получать реальное время)
from datetime import datetime
now = datetime.now()
session = _get_market_session(now.hour, now.minute)

# Объемный анализ (простой)
recent_volume = df["volume"].tail(5).mean()
avg_volume = df["volume"].tail(20).mean()
volume_ratio = recent_volume / max(avg_volume, 1)

# Динамические уровни (без индикаторов)
levels = _calculate_dynamic_levels(df, price)

# Основная логика входа
action, scenario, conf = _universal_entry_logic(
    ticker, price, levels, volume_ratio, session
)
...
atr_val = float(_atr_like(df, n=14).iloc[-1])

# Расчет уровней входа/выхода
if action == "BUY":
    # Вход: у поддержки или текущая цена
    if price <= levels['long_zone_1']:
        entry = price
    else:
        entry = max(price * 0.998, levels['long_zone_1'])  # Небольшой дисконт
    
    # Стоп: 3-5% ниже входа (зависит от типа инструмента)
    sl = entry * (1 - instrument['risk_tolerance'])
    
    # Цели: основаны на целевой прибыли инструмента
    target_profit = instrument['target_profit']
    tp1 = entry * (1 + target_profit * 0.5)   # 50% от цели
    tp2 = entry * (1 + target_profit * 0.8)   # 80% от цели  
    tp3 = entry * (1 + target_profit)         # 100% от цели
    
    alt = "Стратегия overnight: держать до 9:31 AM следующего дня (85% сделок) или при достижении цели."
    
elif action == "SHORT":
    # Вход: у сопротивления или текущая цена
    if price >= levels['short_zone_1']:
        entry = price
    else:
        entry = min(price * 1.002, levels['short_zone_1'])  # Небольшая премия
    ...
    # Цели: вниз от входа
    target_profit = instrument['target_profit']
    tp1 = entry * (1 - target_profit * 0.5)
    tp2 = entry * (1 - target_profit * 0.8)
    tp3 = entry * (1 - target_profit)
    
    alt = "Шорт стратегия: закрытие при достижении цели или утром в 9:31 AM."
    ...
    tp2 = price * 1.04
    tp3 = price * 1.06
    alt = "Ждем правильную временную сессию (10:00-11:30 AM основная) и четкого сетапа."

# Проверка порядка уровней
sl, tp1, tp2, tp3 = _sanity_levels(action, entry, sl, tp1, tp2, tp3, price, atr_val, atr_val, "ST")

# Определение типа ордера
entry_kind = _entry_kind(action, entry, price, atr_val)
entry_label = {
    "buy-stop":"Buy STOP", "buy-limit":"Buy LIMIT", "buy-now":"Buy NOW",
    "sell-stop":"Sell STOP","sell-limit":"Sell LIMIT","sell-now":"Sell NOW"
}.get(entry_kind, "WAIT")
...
chips = []
if volume_ratio > 1.5:
    chips.append("объем выше обычного")
if session == "entry_prime":
    chips.append("основное время входа 10:00-11:30")
elif session == "scalp_window": 
    chips.append("скальпинг 14:00-15:30")
elif session == "exit_window":
    chips.append("время выхода 9:31 AM")

# Добавляем информацию о типе инструмента
chips.append(f"тип: {instrument['type']}")

# Вероятности достижения целей (консервативные)
p1 = _clip01(0.65 + 0.20 * (conf - 0.52) / 0.33)  # TP1 более вероятен
p2 = _clip01(0.45 + 0.15 * (conf - 0.52) / 0.33)  
p3 = _clip01(0.25 + 0.10 * (conf - 0.52) / 0.33)  # TP3 менее вероятен

# Живой текст стратегии
if session == "entry_prime":
    if action == "BUY":
        note_html = "<div style='margin-top:10px; opacity:0.95;'>Основное время входа (10:00-11:30). Покупка на откате от уровней. Цель: держать до утра 9:31 AM.</div>"
    elif action == "SHORT":
        note_html = "<div style='margin-top:10px; opacity:0.95;'>Основное время входа. Шорт от сопротивления. Закрытие утром или по целям.</div>"
    else:
        note_html = "<div style='margin-top:10px; opacity:0.95;'>Основное время, но нет четкого сетапа. Ждем лучшего уровня.</div>"
elif session == "exit_window":
    note_html = "<div style='margin-top:10px; opacity:0.95;'>Время выхода из overnight позиций (9:31 AM). Не время для новых входов.</div>"
elif session == "scalp_window":
    note_html = "<div style='margin-top:10px; opacity:0.95;'>Время скальпинга (14:00-15:30). Быстрые сделки на 2-4 часа.</div>"
else:
    note_html = "<div style='margin-top:10px; opacity:0.95;'>Неактивное время для универсальной стратегии. Ждем 10:00-11:30.</div>"
...
return {
    ...
    "entry_kind": entry_kind,
    "entry_label": entry_label,
    "position_size": _universal_position_size(price),
    "instrument_type": instrument['type'],
    "session": session
}
```

# Функция-роутер для выбора стратегии

def analyze_asset_router(ticker: str, horizon: str, use_universal: bool = False):
“””
Роутер для выбора стратегии:
- use_universal=True: универсальная стратегия (400+ сделок)
- use_universal=False: оригинальная стратегия с AI/Fibonacci
“””
if use_universal and horizon == “Краткосрочно”:
return analyze_asset_universal(ticker, horizon)
else:
return analyze_asset(ticker, horizon)
