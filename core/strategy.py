...
def analyze_asset(ticker: str, horizon: str):
    ...
    # пивоты (ST: weekly; MID/LT: monthly — последняя завершённая)
    ...
    # --- предохранитель "перегрев у порога"
    ...
    # если долго росли и упёрлись в S1/R1/R2 снизу → НЕ покупать
    ...
    # если долго падали и упёрлись сверху в R1/S1/S2 → НЕ шортить
    ...
    # ======== AI override (если есть локальная модель) ========
    try:
    ...
    if score_signal is not None:
        ...
        if out_ai is not None:
            ...
            if p_long >= th_long:
                action = "BUY"
                conf = float(max(0.55, min(0.90, 0.55 + 0.35*(p_long-th_long)/max(1e-9, 1-th_long)))))
    ...
    if action == "BUY":
        ...
        ref = R1 if price >= R1 else price
        entry = min(price, ref - 0.15*step_w) if price >= R1 else price + 0.10*step_d
        sl    = (R1 + 0.60*step_w) if price >= R1 else price + 1.00*step_d
        tp1 = entry - 0.9*step_w; tp2 = entry - 1.6*step_w; tp3 = entry - 2.3*step_w
    ...
    # контекст: «перегрев/перепроданность у порога» (для UI)
    chips = []
    if long_up and (_near_from_below(S1) or _near_from_below(R1) or _near_from_below(R2)):
        chips.append("длинная зелёная серия у порога")
    if long_down and (_near_from_above(R1) or _near_from_above(S1) or _near_from_above(S2)):
        chips.append("длинная красная серия у порога")
    if vol_ratio > 1.05: chips.append("вола растёт")
    if vol_ratio < 0.95: chips.append("вола сжимается")
