...
floor1 = max(min_rr[hz_tag]risk, min_pct[hz_tag]price, atr_mult[hz_tag]atr_val) if abs(tp1 - entry) < floor1: tp1 = entry + side * floor1 floor2 = max(1.6floor1, (min_rr[hz_tag]1.8)risk)
if abs(tp2 - entry) < floor2:
tp2 = entry + side * floor2
min_gap3 = max(0.8floor1, 0.6risk)
...
side = 1 if action == "BUY" else -1
arr = sorted([float(tp1), float(tp2), float(tp3)], key=lambda x: side(x-entry)) d0 = side * (arr[0] - entry) d1 = side * (arr[1] - entry) d2 = side * (arr[2] - entry) if d1 - d0 < eps: arr[1] = entry + side * max(d0 + max(eps, 0.1abs(d0)), d1 + eps)
