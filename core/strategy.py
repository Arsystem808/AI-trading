...
def analyze_asset(ticker: str, horizon: str):
    ...
    # guards: long green series near pivot ⇒ no BUY; long red near pivot ⇒ no SHORT
    ...
    if long_up and (_near_from_below(S1) or _near_from_below(R1) or _near_from_below(R2)):
        ...
        if very_high_pos:
            if (piv.get("R2") is not None) and (price > piv["R2"] + 0.6*buf) and (slope_norm > 0):
                action, scenario = "BUY", "breakout_up"
            else:
                action, scenario = "SHORT", "fade_top"
        else:
            if band >= +2:
                action, scenario = ("BUY","breakout_up") if ((piv.get("R2") is not None) and (price > piv["R2"] + 0.6*buf) and (slope_norm > 0)) else ("SHORT","fade_top")
            elif band == +1:
    ...
    # ===== AI override (если подключена локальная модель) =====
    try:
    ...
    if action == "BUY":
        ...
        if (piv.get("R1") is not None) and (price >= piv["R1"]):
            entry = min(price, piv["R1"] - 0.15*step_w)
            sl    = piv["R1"] + 0.60*step_w
        else:
