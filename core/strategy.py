...
def _last_period_hlc(df: pd.DataFrame, rule: str):
    ...
    if len(g) < 2:
        return None
    row = g.iloc[-2]  # последняя завершённая неделя/месяц
...
# ---------- ограничение TP3 с учётом тренда ----------
def _cap_tp3_trend_aware(tp1, tp2, tp3, action, hz, price, piv, slope_norm, band, step_w):
    """
    Делает TP3 реалистичным: ограничивает глубину в сильном ап-тренде (для SHORT)
    и высоту в сильном даун-тренде (для BUY). Чуть поджимает TP2, чтобы
    сохранялась «лестница».
    """
    cap_pct = {"ST": 0.06, "MID": 0.09, "LT": 0.12}  # макс. удаление TP3 от текущей цены

    if action == "SHORT":
        p = float(piv.get("P", price))
        s1 = piv.get("S1", None)
        # Базовый «пол» для TP3 (не глубже):
        lb = price * (1.0 - cap_pct[hz])
        lb = max(lb, p - 0.6 * step_w)
        if s1 is not None:
            lb = max(lb, float(s1) - 0.3 * step_w)
        # В ап-тренде у верха — ещё консервативнее
        if slope_norm > 0 and band >= +1:
            lb = max(lb, p - 0.2 * step_w)

        if tp3 < lb:
            tp3 = lb
        if tp2 < tp3 + 0.3 * step_w:
            tp2 = tp3 + 0.3 * step_w

    elif action == "BUY":
        p = float(piv.get("P", price))
        r1 = piv.get("R1", None)
        # Базовый «потолок» для TP3 (не выше):
        ub = price * (1.0 + cap_pct[hz])
        ub = min(ub, p + 0.6 * step_w)
        if r1 is not None:
            ub = min(ub, float(r1) + 0.3 * step_w)
        # В даун-тренде у дна — ещё консервативнее
        if slope_norm < 0 and band <= -1:
            ub = min(ub, p + 0.2 * step_w)
...
def _order_targets(entry: float, tp1: float, tp2: float, tp3: float, action: str, eps: float = 1e-6):
    ...
    # Гарантируем, что цели по правильную сторону от entry
    for i in range(3):
        if side * (arr[i] - entry) <= eps:
            arr[i] = entry + side * max(eps, 0.001 * abs(entry) + 1e-6)
...
def analyze_asset(ticker: str, horizon: str):
    ...
    df = cli.daily_ohlc(ticker, days=days)  # index: DatetimeIndex
    ...
    if very_high_pos:
        ...
        if band >= +2:
            action, scenario = ("BUY", "breakout_up") if (
                (R2 is not None) and (price > R2 + 0.6 * buf) and (slope_norm > 0)
            ) else ("SHORT", "fade_top")
        elif band == +1:
            action, scenario = (
                ("WAIT", "upper_wait")
    ...
    if action == "BUY":
        ...
        if price >= R1:
            entry = min(price, R1 - 0.15 * step_w)  # после отказа — не по рынку
            sl = R1 + 0.60 * step_w
        else:
    ...
    # 2) ограничитель TP3 с учётом тренда (и лёгкая коррекция TP2)
    tp1, tp2, tp3 = _cap_tp3_trend_aware(tp1, tp2, tp3, action, hz, price, piv, slope_norm, band, step_w)

    # 3) строгий порядок целей + защита стороны
    ...
    if streak >= 3:
        chips.append(f"{streak} зелёных подряд")
    if streak <= -3:
        chips.append(f"{abs(streak)} красных подряд")
    if long_upper:
        chips.append("тени сверху")
    if long_lower:
        chips.append("тени снизу")
    ...
    if action == "BUY":
        lead = rng.choice(
            ...
            ["Поддержка снизу — вход аккуратно, без погони.", "Отталкиваемся от опоры — беру после паузы."]
        )
    elif action == "SHORT":
        lead = rng.choice(
            [
    ...
    if pos >= 0.8 and action != "BUY":
        adds.append("Погоня сверху — не мой вход. Жду откат.")
    if action == "BUY" and pos <= 0.4:
        adds.append("Вход только после реакции, без рывка.")
    if streak >= 4:
        adds.append("Серия длинная — риски отката выше.")
