...
def _heikin_ashi(ohlc: pd.DataFrame) -> pd.DataFrame:
    """Возвращает DataFrame с колонками ha_open, ha_close, ha_high, ha_low."""
...
def _ha_streak(ha: pd.DataFrame) -> int:
    """Положительная (зелёная) серия — >0; отрицательная (красная) — <0."""
...
def _macd_decelerating(hist: pd.Series, look: int = 4) -> bool:
    """Усталость: в положительной зоне столбики последовательно уменьшаются;
    в отрицательной — растут (по модулю уменьшаются)."""
    if len(hist) < look + 1:
        return False
    tail = hist.tail(look)
    if (tail > 0).all():
...
def _rsi_percentile_flag(rsi: pd.Series, upper_q=0.8, lower_q=0.2, window=200):
    """Динамический фильтр 'перегрет/перепродан' по перцентилям последней истории."""
    if len(rsi) < 30:
        return False, False
...
def _horizon_cfg(text: str):
    """ST: weekly pivots; MID: monthly pivots; LT: monthly pivots."""
    if "Кратко" in text:
        return dict(
            look=60, trend=14, atr=14, pivot_period="W-FRI", use_weekly_atr=False
        )
    if "Средне" in text:
        return dict(
            look=120, trend=28, atr=14, pivot_period="M", use_weekly_atr=True
        )
    # LT
...
def analyze_asset(ticker: str, horizon: str):
    ...
    # --- данные (дневные) ---
    days = max(360, cfg["look"] * 4)  # побольше истории для W/M расчётов
    df = cli.daily_ohlc(ticker, days=days)  # DatetimeIndex
    price = cli.last_trade_price(ticker)

    # --- производные ТФ для индикаторов/подтверждений ---
    ...
    # --- позиция в диапазоне (по дневкам) ---
    ...
    # =========================
    # HA / MACD / RSI (base & senior)
    # =========================
    ha_base = _heikin_ashi(base_df)
    ha_stk = _ha_streak(ha_base)
    macd_base = _macd_hist(base_df["close"])
    macd_stk = _macd_streak(macd_base)
    ...
    macd_sen = _macd_hist(senior_df["close"])
    macd_sen_sign = np.sign(float(macd_sen.iloc[-1]))

    # Пороги серий по горизонту
    ...
    if overheat_top:
        ...
        if very_high_pos:
            ...
            if band >= +2:
                ...
                action, scenario = (
                    ("WAIT", "upper_wait")
                    if (slope_norm > 0.0015 and not bearish_reject and not long_upper)
                    else ("SHORT", "fade_top")
                )
            elif band == 0:
    ...
    # =========================
    # БАЗОВАЯ УВЕРЕННОСТЬ + подтверждение старшего ТФ
    # =========================
    ...
    # Подтверждение старшего ТФ (цвет HA и знак MACD)
    if action == "BUY":
        if (ha_sen_color > 0) and (macd_sen_sign >= 0):
            base_conf += 0.05
        if rsi_oversold:
            base_conf += 0.02
        if overheat_top:
    ...
    if score_signal is not None:
        feats = dict(
            ...
            vol_ratio=vol_ratio,
            streak=float(simple_streak),
            band=float(band),
            long_upper=bool(long_upper),
            long_lower=bool(long_lower),
            ha_streak=float(ha_stk),
            macd_streak=float(macd_stk),
            macd_tired=bool(macd_tired),
        ...
        if out_ai is not None:
            ...
            if p_long >= th_long:
                action_ai = "BUY"
                conf_ai = 0.55 + 0.35 * (p_long - th_long) / max(1e-9, 1.0 - th_long)
            elif p_long <= th_short:
                action_ai = "SHORT"
                conf_ai = 0.55 + 0.35 * ((th_short - p_long) / max(1e-9, th_short))
            else:
                action_ai = "WAIT"
                conf_ai = 0.55 - 0.07
            action = action_ai
            conf = float(max(0.55, min(0.90, conf_ai)))
    # ======== конец AI override ========
    ...
    if action == "BUY":
        if scenario == "exhaustion_bottom":
            if near_S3:
                entry = max(price, S3 + 0.15 * step_w)
                sl = S3 - 0.80 * step_w
                tp1, tp2, tp3 = S2, P, R1
            elif near_S2:
                entry = max(price, S2 + 0.15 * step_w)
                sl = S2 - 0.80 * step_w
                tp1, tp2, tp3 = P, R1, R2
            else:
        ...
        if scenario == "overheat_top":
            if near_R3:
                entry = min(price, R3 - 0.15 * step_w)
                sl = R3 + 0.80 * step_w
                tp1, tp2, tp3 = R2, P, S1
            elif near_R2:
                entry = min(price, R2 - 0.15 * step_w)
                sl = R2 + 0.80 * step_w
                # цель 1: между P и S1 (консервативно), цель 2: S1/S2
                mid_PS1 = (P + S1) / 2.0
    ...
    # Вероятности достижения целей — от уверенности
    ...
    chips = []
    if pos >= 0.8:
        chips.append("верхний диапазон")
    elif pos >= 0.6:
        chips.append("под верхним краем")
    elif pos >= 0.4:
        chips.append("средняя зона")
    elif pos >= 0.2:
        chips.append("нижняя половина")
    else:
        chips.append("ниже опоры")
    if vol_ratio > 1.05:
        chips.append("волатильность растёт")
    if vol_ratio < 0.95:
        chips.append("волатильность сжимается")
    if simple_streak >= 3:
        chips.append(f"{simple_streak} зелёных подряд")
    if simple_streak <= -3:
        chips.append(f"{abs(simple_streak)} красных подряд")
    if ha_stk >= ha_thr:
        chips.append(f"HA-серия {ha_stk}")
    if ha_stk <= -ha_thr:
        chips.append(f"HA-серия {abs(ha_stk)} красных")
    if macd_tired:
        chips.append("MACD: усталость")

    # Живой текст — упоминания HA/MACD/RSI
    seed = int(
        hashlib.sha1(f"{ticker}{df.index[-1].date()}".encode()).hexdigest(), 16
    ) % (2**32)
    ...
    if action == "BUY":
        lead = rng.choice(lead_buy)
        if exhaustion_bottom:
            lead += " Перегиб вниз иссяк — играю от восстановления."
        if rsi_oversold:
            lead += " RSI внизу поддерживает идею."
    elif action == "SHORT":
        lead = rng.choice(lead_short)
        if overheat_top:
            lead += " Перегрев у крыши — беру слабость."
        if rsi_overbought:
            lead += " RSI сверху — риск разворота повышен."
    else:
        lead = rng.choice(lead_wait)
