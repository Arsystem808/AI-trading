...
def _horizon_cfg(text: str):
    """
    Уровни для сделок:
      ST — пивоты прошлой недели
      MID — пивоты прошлого месяца
      LT — пивоты прошлого года
    Интуиция (HA/MACD) — всегда по недельным барам.
    """
    if "Кратко" in text:
...
def _cap_targets_by_trend(tp1, tp2, tp3, *, action: str, piv: dict, step: float,
    ...
    """Сдерживаем цели против сильного тренда."""
...
def analyze_asset(ticker: str, horizon: str):
    ...
    # недельная агрегация (для HA/MACD и weekly-pivots)
    ...
    # ---- интуитивный weekly-контроль «у крыши/дна» ----
    # если weekly показывает крышу + длинные зелёные — не лонг
    ...
    # если weekly показывает дно + длинные красные — не шорт
    ...
    if score_signal is not None:
        feats = dict(
            ...
            vol_ratio=vol_ratio, streak=float(streak_px), band=float(band_hz),
            long_upper=bool(long_upper), long_lower=bool(long_lower),
            ha_streak=float(ha_streak), macd_streak=float(macd_streak),
            trend_score=float(trend_score), weekly_band=float(band_w)
        )
    ...
    # уровни (используем пивоты горизонта — как «рабочие»)
    ...
    # ФИКСЫ и защита против тренда (по weekly-пивотам!)
    atr_for_floor = atr_w if hz != "ST" else atr_d
    tp1, tp2, tp3 = _apply_tp_floors(entry, sl, tp1, tp2, tp3, action, hz, price, atr_for_floor)
    tp1, tp2, tp3 = _cap_targets_by_trend(tp1, tp2, tp3, action=action, piv=piv_w,
                                          step=step_w, trend_score=trend_score, band=band_w)
    ...
    if long_red   and action == "BUY":   adds.append("Длинная красная — осторожно.")
