...
def _horizon_cfg(text: str):
    # уровни берём от weekly pivot; weekly ATR — для MID/LT
    if "Кратко" in text:
...
def _apply_tp_floors(entry: float, sl: float, tp1: float, tp2: float, tp3: float,
    ...
    if action not in ("BUY", "SHORT"):  # WAIT
        return tp1, tp2, tp3
    risk = abs(entry - sl)
    if risk <= 1e-9:
        return tp1, tp2, tp3
...
def analyze_asset(ticker: str, horizon: str):
    ...
    # weekly MACD (фон)
    w_cl = df["close"].resample("W-FRI").last().dropna()
    macd_w_hist = float(_macd_hist(w_cl).iloc[-1]) if len(w_cl) > 35 else 0.0
    ...
    if score_signal is not None:
        feats = dict(
            ...
            vol_ratio=vol_ratio, streak=float(streak),
            band=float(band), long_upper=bool(long_upper), long_lower=bool(long_lower),
        )
    ...
    if action == "BUY":
        if scenario == "breakout_up":
            ...
            sl    = base_ref - (1.0 if hz == "ST" else 1.2) * step_w
            tp1, tp2, tp3 = _three_targets_from_pivots(entry, "BUY", piv, step_w)
        elif price < P:
            entry = max(price, piv["S1"] + 0.15 * step_w); sl = piv["S1"] - (0.8 if hz == "ST" else 1.0) * step_w
            tp1, tp2, tp3 = _three_targets_from_pivots(entry, "BUY", piv, step_w)
        else:
            entry = max(price, P + 0.10 * step_w); sl = P - (0.8 if hz == "ST" else 1.0) * step_w
            tp1, tp2, tp3 = _three_targets_from_pivots(entry, "BUY", piv, step_w)
        ...
        if price >= R1:
            ...
            sl    = R1 + (0.8 if hz == "ST" else (1.1 if hz == "MID" else 1.3)) * step_w
            tp1, tp2, tp3 = _three_targets_from_pivots(entry, "SHORT", piv, step_w)
        else:
            entry = price + 0.15 * step_d
            sl    = price + (0.8 if hz == "ST" else 1.0) * step_d
            tp1, tp2, tp3 = _three_targets_from_pivots(entry, "SHORT", piv, step_w)
    ...
    # ---------- ТРЕНД-ОСОЗНАННЫЕ КЭПЫ ----------
    is_up_trend   = (slope_norm > 0) and (price >= P) and (macd_w_hist >= 0)
    is_down_trend = (slope_norm < 0) and (price <= P) and (macd_w_hist <= 0)

    # Шорт у крыши в ап-тренде — ручная расстановка более «человечных» целей
    if action == "SHORT" and price >= R1 and is_up_trend:
        tp1 = R1 - 0.6 * step_w
        tp2 = P
        tp3 = max(P - 0.4 * step_w, tp2 - 0.4 * step_w)

    # Общие кэпы глубины/высоты TP2/TP3
    if action == "SHORT" and is_up_trend:
        tp2 = max(tp2, P)                  # не глубже центра
        tp3 = max(tp3, P - 0.5 * step_w)   # не уводим в «подвал»
    if action == "BUY" and is_down_trend:
        tp2 = min(tp2, P)                  # консервативнее
        if R1 is not None:
            tp3 = min(tp3, R1 + 0.5 * step_w)
    ...
    if is_up_trend: chips.append("старший фон: бычий")
    if is_down_trend: chips.append("старший фон: медвежий")
