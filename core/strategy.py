...
# ---------- horizons (ST=weekly, MID=monthly, LT=monthly) ----------
def _horizon_cfg(text: str):
    if "Кратко" in text:   # 1–5 дней
        return dict(look=120, trend=14, atr=14, pivot_rule="W-FRI", ha_macd_rule="W-FRI", use_weekly_atr=False, tag="ST")
    if "Средне" in text:   # 1–4 недели
        return dict(look=240, trend=28, atr=14, pivot_rule="M",     ha_macd_rule="W-FRI", use_weekly_atr=True,  tag="MID")
    # ДОЛГОСРОК -> МЕСЯЧНЫЕ ПИВОТЫ
    return dict(look=540, trend=56, atr=14, pivot_rule="M",         ha_macd_rule="W-FRI", use_weekly_atr=True,  tag="LT")

def _hz_tag(text: str) -> str:
    return _horizon_cfg(text)["tag"]
...
def _heikin_ashi(df_ohlc: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(df_ohlc)):
        ha.iloc[i,0] = float(df_ohlc["open"].iloc[0]) if i==0 else (ha["open"].iloc[i-1] + ha["close"].iloc[i-1]) / 2.0
...
def _weekly_trend_score(df_w: pd.DataFrame) -> float:
    ...
    hist = float(_macd_hist_weekly(df_w).iloc[-1])
    hist_norm = hist / (abs(float(df_w["close"].pct_change().tail(26).std())) + 1e-9)
    return float(max(-1.0, min(1.0, 0.65*np.tanh(180*slope) + 0.35*np.tanh(hist_norm))))
...
def analyze_asset(ticker: str, horizon: str):
    ...
    if long_green and band_w >= +1:
        # у weekly-крыши после длинного бычьего — точно НЕ LONG
        if (piv_w.get("R2") is not None) and (price <= piv_w["R2"] + 0.8*buf_w):
            action, scenario = "SHORT", "fade_top_confirmed"
        else:
            action, scenario = "WAIT", "overheat_top"

    if long_red and band_w <= -1:
        if (piv_w.get("S2") is not None) and (price >= piv_w["S2"] - 0.8*buf_w):
            action, scenario = "BUY", "rebound_confirmed"
        else:
            action, scenario = "WAIT", "washout_bottom"

    # дополнительный стоп-кран: если всё равно получился LONG у крыши/SHORT у дна
    if action=="BUY"  and long_green and band_w >= +1: action="WAIT"
    if action=="SHORT" and long_red   and band_w <= -1: action="WAIT"
    ...
    if action == "BUY":
        ...
        if price >= R1:                 # confirm: не ниже текущей
            entry = price; sl = R1 + 0.60*step_w
        else:
    ...
    # жёсткие пакеты целей
    if action=="SHORT" and trend_score > 0.45 and band_w >= +1:
        tp1,tp2,tp3 = _pack_short_at_top_weekly(piv_w, price, atr_w)
    if action=="BUY" and trend_score < -0.45 and band_w <= -1:
        tp1,tp2,tp3 = _pack_long_at_bottom_weekly(piv_w, price, atr_w)
    ...
    # context chips (+быстрый дебаг)
    chips = [("верхний диапазон" if pos>=0.8 else
              "под верхним краем" if pos>=0.6 else
              "средняя зона" if pos>=0.4 else
              "нижняя половина" if pos>=0.2 else "ниже опоры")]
    ...
    chips.append(f"pivot:{cfg['pivot_rule']}")  # маленький отладочный маркер
