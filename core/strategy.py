...
def _get_m7_cal() -> ConfidenceCalibrator:
    return CAL_CONF.get("M7", ConfidenceCalibrator("identity"))
...
class _M7Predictor:
    ...
    def _predict_proba_safe(self, X: np.ndarray, df_last_row: Optional[pd.DataFrame]) -> Optional[np.ndarray]:
        # Сначала пробуем вектор признаков (1, d), затем fallback на df.tail(1) для табличных пайплайнов
        try:
    ...
    def predict(self, df: pd.DataFrame, price: float, atr: float) -> Tuple[str, float]:
        try:
            ...
            if hasattr(self.model, "predict_proba"):
                proba = self._predict_proba_safe(X, df1)
            ...
            # многокласс: BUY/SHORT/WAIT по argmax + калибровка уверенности
            acts = ["BUY","SHORT","WAIT"]
