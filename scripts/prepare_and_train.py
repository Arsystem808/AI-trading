...
def prepare_training_data(ticker, from_date, to_date):
...
def train_and_save_model(ticker, df):
    ...
    model.fit(X_train, y_train, eval_set=[(X_test, y_test)], early_stopping_rounds=50, verbose=20)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Скачать данные, подготовить и обучить модель M7pro на Polygon")
    parser.add_argument("--ticker", type=str, required=True, help="Тикер, например SPY")
    parser.add_argument("--from_date", type=str, required=True, help="Дата начала в формате YYYY-MM-DD")
    parser.add_argument("--to_date", type=str, required=True, help="Дата конца в формате YYYY-MM-DD")
