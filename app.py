...
# =====================
# Admin · Train ST model inline (safe block)
# =====================
def render_training_panel():
    with st.expander("🧠 ML · быстрый тренинг (ST) прямо здесь"):
        st.caption("Обучит краткосрочную модель (ST) и сохранит файл в models/. Затем файл можно скачать и загрузить в репозиторий.")
        ...
        with open(out_path, "rb") as f:
            st.download_button("⬇️ Скачать модель (ST)", data=f.read(), file_name="arxora_lgbm_ST.joblib")
