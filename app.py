# app.py — Arxora (AI) - Updated with Universal Strategy
import os
import re
import hashlib
import random
...
from core.strategy import analyze_asset, analyze_asset_universal
...
“universal_entry_prime”: [“Основное время входа (10:00-11:30). Стратегия основана на анализе 400+ успешных сделок. Рекомендация: держать до утра 9:31 AM или по достижении цели.”],
“universal_scalp”: [“Время скальпинга (14:00-15:30). Быстрые позиции на 2-4 часа. Стратегия проверена на сотнях сделок.”],
“universal_exit”: [“Время выхода из overnight позиций (9:31 AM). Не время для новых входов - лучше закрывать существующие позиции.”]
},
“STOPLINE”: [
...
out = analyze_asset_universal(ticker=symbol_for_engine, horizon=horizon)
else:
...
```
    ...
    if action in ("BUY", "SHORT"):
        ...
        if use_universal and 'position_size' in out:
            st.markdown(
                ...
                f"Торговая сессия: {out.get('session', 'N/A')}"
                f"</div>",
                unsafe_allow_html=True,
            )
    ...
    if use_universal and 'session' in out:
        session = out['session']
