# -*- coding: utf-8 -*-
# Arxora Trading Platform v15.0 — FINAL PRODUCTION
# All critical UX issues fixed: buttons with blue dot, clean inputs, stable sidebar
...
.stTextInput input, .stNumberInput input {
  ...
  color: #fff !important;
  border: 1px solid rgba(91,127,249,0.5) !important;
...
.stTextInput input:focus, .stNumberInput input:focus {
  ...
  box-shadow: 0 0 0 2px rgba(91,127,249,0.2) !important;
}
.stTextInput input::placeholder {color: #808080 !important; opacity: 1 !important;}

/* Container overflow fix for "Press Enter…" */
...
[data-testid="stPasswordInput"] {overflow: hidden !important;}
[data-testid="stPasswordInput"] > div {overflow: hidden !important; border-radius: 12px !important;}
...
.stButton > button[kind="primary"]:hover {background: #4a6df0 !important;}
.stButton > button[kind="secondary"] {background: #ea3943 !important; color: #fff !important;}

/* ===== Model Selector Buttons (with dot) ===== */
.model-button {
  display: inline-flex;
  align-items: center;
  gap: 0.6rem;
...
def normalize_ai_override(override: float) -> float:
    # Map [-50,+50] to [0,100]
    return max(0, min(100, 50 + override))
...
def render_signal_card(action: str, ticker: str, price: float, conf_pct: float, rules_conf: float, levels: Dict, output: Dict, model_name: str):
    ...
    bar_width = normalize_ai_override(ai_override)
    st.markdown(f"""
...
risk control and plan revision essential if consolidation occurs {"below" if action=="BUY" else "above"} zone.
...
with tabs[0]:
    ...
    # MODEL SELECTOR: Buttons with blue dot
    st.write("**Model**")
    ...
    for i, m in enumerate(models):
        with cols[i]:
            ...
            # HTML button with dot
            button_html = f"""
            <div class="model-button {active_class}" onclick="document.getElementById('model_{m}').click();">
            ...
            if st.button(m, key=f"model_{m}", label_visibility="collapsed"):
...
with tabs[1]:
    ...
    if "last_signal" in st.session_state:
        ...
        if sig["action"] not in ("BUY","SHORT"):
            ...
            """, unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("Add Trade to Portfolio", type="primary", use_container_width=True, key="add_port_btn"):
                try:
                    probs = sig["output"].get('probs') or {}
                    data = {
                        'ticker': sig["ticker"],
