...
def card_html(title, value, sub=None, color=None):
    bg = "#141a20"
    if color == "green": bg = "#10b28d"
    elif color == "red": bg = "#b21035"
    return f"""
...
if run and ticker:
    try:
        ...
        st.markdown(
            f"""
            <div style="background:#c0933f; padding:14px 16px; border-radius:16px; border:1px solid rgba(255,255,255,0.06); margin-bottom:10px;">
