# -*- coding: utf-8 -*-
# Arxora Trading Platform v16.3 — FINAL ULTRA PRODUCTION
# With large radio dots + narrower uniform cards
...
.stTextInput input::placeholder {color: transparent !important; opacity: 0 !important;}
...
/* ===== LARGE RADIO DOTS (Vertical List) ===== */
.stRadio > div {
  display: flex;
...
/* Large Radio Dot - UNSELECTED (gray) */
...
.stRadio > div > label:hover::before {
  ...
  background: rgba(91,127,249,0.1);
}

/* SELECTED - Large Blue Dot with White Center */
.stRadio > div > label[data-checked="true"] {
  color: #fff !important;
  border: 2px solid var(--accent-blue) !important;
  background: rgba(91,127,249,0.08) !important;
}
...
.stRadio > div > label[data-checked="true"]::before {
  ...
  width: 16px;
  height: 16px;
  box-shadow: 0 0 12px rgba(91,127,249,0.8), inset 0 0 4px rgba(255,255,255,0.6);
}
...
with tabs[1]:
    ...
    if "last_signal" in st.session_state:
        ...
        if sig["action"] not in ("BUY","SHORT"):
            ...
            if pd:
                df_tp = pd.DataFrame([
                    {"Level": "TP1", "Price": f"${sig['levels']['tp1']:.2f}", "Probability": f"{int(probs.get('tp1',0)*100)}%", "P&L": f"{abs(sig['levels']['tp1']-sig['levels']['entry'])/max(1e-9,sig['levels']['entry'])*100:.2f}%"},
                    {"Level": "TP2", "Price": f"${sig['levels']['tp2']:.2f}", "Probability": f"{int(probs.get('tp2',0)*100)}%", "P&L": f"{abs(sig['levels']['tp2']-sig['levels']['entry'])/max(1e-9,sig['levels']['entry'])*100:.2f}%"},
                    {"Level": "TP3", "Price": f"${sig['levels']['tp3']:.2f}", "Probability": f"{int(probs.get('tp3',0)*100)}%", "P&L": f"{abs(sig['levels']['tp3']-sig['levels']['entry'])/max(1e-9,sig['levels']['entry'])*100:.2f}%"}
                ])
...
<div style="text-align:center;color:#707070;font-size:12px;padding:2rem 0 1rem;border-top:1px solid rgba(255,255,255,0.05);margin-top:2rem;">
  Arxora v16.3 · Professional Trading Intelligence. AI signals informational only.
</div>
""", unsafe_allow_html=True)
