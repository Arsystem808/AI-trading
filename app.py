...
def card_html(title: str, value: str, sub: Optional[str]=None, color: Optional[str]=None) -> str:
    bg = "#141a20"
    if color == "green": bg = "#2c8a02"
    elif color == "red": bg = "#8a022c"
    return f"""
...
with tab_signals:
    ...
    if run and ticker:
        try:
            ...
            if action == "BUY": bg = "linear-gradient(98deg, #2c8a02, #2c8a02)"
            elif action == "SHORT": bg = "linear-gradient(98deg, #8a022c, #8a022c)"
            st.markdown(f"""
