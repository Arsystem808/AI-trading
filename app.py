...
if run and ticker:
    try:
        ...
        # === BEGIN: breakdown block (детальная раскладка) ===
        def render_breakdown(out: dict, ticker: str):
            # если нет данных — ничего не рисуем
            if not out or "levels" not in out:
                return
            ...
            ctx = out.get("context", [])
            entry_kind = out.get("entry_kind", "")
            entry_label = out.get("entry_label", "")
            ...
            if ctx:
                chips_html = " · ".join(ctx)
                chips_html = f"<div style='opacity:0.8; margin-top:8px; font-size:0.9rem;'>{chips_html}</div>"

            # probs neat
            p1 = int(round(float(probs.get("tp1", 0.0)) * 100))
            p2 = int(round(float(probs.get("tp2", 0.0)) * 100))
            p3 = int(round(float(probs.get("tp3", 0.0)) * 100))

            # entry / kind line
            entry_line = f"<div style='font-size:0.95rem; opacity:0.9; margin-top:6px;'>"
            if rec.get("action") in ("BUY","SHORT"):
                entry_line += f"<b>{entry_label}</b> · {float(lv.get('entry', 0.0)):.2f} · <span style='opacity:0.75;'>{entry_kind}</span>"
            else:
                entry_line += f"<b>Нет сигнала</b>"
            entry_line += "</div>"

            # levels table (мини-карточки)
            levels_html = f"""
            <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:10px;">
              <div style="background:rgba(255,255,255,0.02);padding:10px;border-radius:10px;min-width:140px;">
                <div style="font-size:0.8rem;opacity:0.8">Entry</div>
                <div style="font-weight:700;margin-top:6px;color:#8ED1FF">{float(lv.get('entry',0.0)):.2f}</div>
              </div>
              <div style="background:rgba(255,0,0,0.06);padding:10px;border-radius:10px;min-width:140px;">
                <div style="font-size:0.8rem;opacity:0.8">Stop Loss</div>
                <div style="font-weight:700;margin-top:6px;color:#FF8A8A">{float(lv.get('sl',0.0)):.2f}</div>
              </div>
              <div style="background:rgba(0,255,0,0.02);padding:10px;border-radius:10px;min-width:140px;">
                <div style="font-size:0.8rem;opacity:0.8">TP1 · {p1}%</div>
                <div style="font-weight:700;margin-top:6px;color:#8EF08E">{float(lv.get('tp1',0.0)):.2f}</div>
              </div>
              <div style="background:rgba(0,255,0,0.02);padding:10px;border-radius:10px;min-width:140px;">
                <div style="font-size:0.8rem;opacity:0.8">TP2 · {p2}%</div>
                <div style="font-weight:700;margin-top:6px;color:#8EF08E">{float(lv.get('tp2',0.0)):.2f}</div>
              </div>
              <div style="background:rgba(0,255,0,0.02);padding:10px;border-radius:10px;min-width:140px;">
                <div style="font-size:0.8rem;opacity:0.8">TP3 · {p3}%</div>
                <div style="font-weight:700;margin-top:6px;color:#8EF08E">{float(lv.get('tp3',0.0)):.2f}</div>
              </div>
            </div>
            """

            # alt and note
            alt_html = f"<div style='margin-top:8px; font-size:0.9rem; opacity:0.85;'><b>Альт. сценарий:</b> {alt}</div>" if alt else ""
            note_block = note_html if note_html else ""

            # assemble
            html = f"""
            <div style="margin-top:12px; padding:12px 14px; border-radius:12px; background:rgba(255,255,255,0.015);">
              {entry_line}
              {chips_html}
              {levels_html}
              <div style='margin-top:8px; font-size:0.9rem; color: #FFA94D; font-weight:600;'>{rr_line(lv)}</div>
              {alt_html}
              {note_block}
            </div>
            """
