...
def card_html(title: str, value: str, sub: Optional[str]=None, color: Optional[str]=None) -> str:
    bg = "#141a20"
    if color == "green": bg = "#022e90"
    elif color == "red": bg = "#90022e"
    return f"""
...
with tab_signals:
    ...
    if run and ticker:
        try:
            ...
            if action == "BUY": bg = "linear-gradient(98deg, #2e9002, #2e9002)"
            elif action == "SHORT": bg = "linear-gradient(98deg, #90022e, #90022e)"
            st.markdown(f"""
