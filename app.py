...
# -------- Optional performance API stubs
try:
...
if run and ticker:
    try:
        ...
        CUSTOM_PHRASES = {
            ...
            "DISCLAIMER": "AI‑анализ носит информационный характер и не является инвестрекомендацией; прошлые результаты не гарантируют будущие."
        }
...
st.markdown(
    ...
    <div style="background-color: #000000; color: #ffffff; padding: 15px; border-radius: 10px; margin-top: 6px;">
        ...
        Arxora AI — ансамбль агентов с калибровкой уверенности; анализ не является инвестсоветом.
        </p>
    </div>
    """,
    unsafe_allow_html=True
