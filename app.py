...
from core.strategy import analyze_asset           # основной движок (AI + правила)
...
# Режим (AI / Rules)
try:
    from core import ai_inference  # noqa
    st.caption("Mode: **AI** (модели найдены)")
except Exception:
    st.caption("Mode: **Rules** (работает только правилами)")
...
if run and ticker:
    try:
        ...
        # --- подпись входа (берём из стратегии, либо считаем) ---
        entry_label = out.get("entry_label")
        if not entry_label:
            entry_label = infer_entry_label(action, entry, price_now)
        ...
        if action in ("BUY", "SHORT"):
            ...
            with c3:
                st.markdown(card_html("TP 1", f"{tp1:.2f}", sub=f"Probability {int(round(out['probs']['tp1']*100))}%",), unsafe_allow_html=True)

            c1, c2 = st.columns(2)
            with c1:
                st.markdown(card_html("TP 2", f"{tp2:.2f}", sub=f"Probability {int(round(out['probs']['tp2']*100))}%"), unsafe_allow_html=True)
            with c2:
                st.markdown(card_html("TP 3", f"{tp3:.2f}", sub=f"Probability {int(round(out['probs']['tp3']*100))}%"), unsafe_allow_html=True)
...
# (ниже — как в твоей версии; без изменений по логике)
# =========================================================
...
def _snapshot_decision(df_up_to: pd.DataFrame, horizon_text: str):
    ...
    def _linreg_slope_local(y: np.ndarray) -> float:
        n = len(y)
        if n < 2: return 0.0
        x = np.arange(n, dtype=float); xm, ym = x.mean(), y.mean()
        denom = ((x - xm) ** 2).sum()
    ...
    slope = _linreg_slope_local(closes.tail(cfg["trend"]).values)
    ...
    def _streak_local(cl: pd.Series) -> int:
        s = 0
        for i in range(len(cl)-1, 0, -1):
            d = cl.iloc[i] - cl.iloc[i-1]
            if d > 0:
                if s < 0: break
                s += 1
            elif d < 0:
                ...
                break
        return s
    streak    = _streak_local(closes)

    last_row = df_up_to.iloc[-1]
    def _wick_profile_local(row):
        o, c, h, l = row["open"], row["close"], row["high"], row["low"]
        body = abs(c - o)
        upper = max(0.0, h - max(o, c))
        lower = max(0.0, min(o, c) - l)
        return body, upper, lower
    body, upper, lower = _wick_profile_local(last_row)
...
def _simulate_outcome(df_all: pd.DataFrame, start_idx: int, action: str,
    ...
    for k in range(1, max_days + 1):
        ...
        tps_today = [(lab, px) for (lab, hit, px) in tps if hit]
        ...
        if exit_policy == "first_touch":
            ...
            for lab, px in tps_today:  # TP3 -> TP1
