# -*- coding: utf-8 -*-
# app.py — Arxora Trading Platform v14.1 (BLUE STANDARDIZED)
...
:root {
    ...
    --accent-blue: #5B7FF9;         /* Core brand accent */
    --success: #16c784;
    --danger: #ea3943;
    --warning: #ffa94d;
    --text-primary: #ffffff;
...
/* Inputs — unified blue focus, single border */
...
/* Hide Streamlit container borders */
.stTextInput > div, .stNumberInput > div { border: none !important; box-shadow: none !important; }

/* Press Enter hint fix + password eye alignment */
[data-testid="stTextInput"] div[title*="Press Enter to apply"],
[data-testid="stNumberInput"] div[title*="Press Enter to apply"],
span[data-baseweb="tag"] div[title*="Press Enter to apply"] {
    display: none !important;
}
[data-testid="stTextInput"] svg[data-testid="visibility-off-icon"],
[data-testid="stTextInput"] svg[data-testid="visibility-on-icon"] {
    transform: translateX(-4px);  /* keeps eye inside field */
}

/* Buttons — standardized */
.stButton > button {
    padding: 0.75rem 1.5rem !important;
    background: var(--accent-blue) !important;   /* default = blue */
    color: #fff !important;
...
/* Radio — show selected as blue button */
.stRadio > div {
    display: flex;
    gap: 0.75rem;
    flex-wrap: wrap;
...
.stRadio > div > label::before {
    ...
    background: transparent; border: 2px solid var(--text-tertiary);
...
.stRadio > div > label[data-checked="true"]::before {
    background: #ffffff; border-color: #ffffff; box-shadow: 0 0 6px rgba(255,255,255,0.7);
}

/* Tabs */
.stTabs [data-baseweb="tab-list"] {
    gap: 2rem; border-bottom: 1px solid var(--border); padding-bottom: 0 !important;
}
.stTabs [data-baseweb="tab"] {
    color: var(--text-tertiary); font-weight: 700; text-transform: uppercase;
    font-size: 11px; letter-spacing: 1px; padding: 1rem 0 !important; background: transparent !important;
}
...
[data-testid="stMetric"] label {
    ...
    font-size: 10px !important; letter-spacing: 0.5px !important; font-weight: 700 !important;
}
[data-testid="stMetric"] [data-testid="stMetricValue"] {
    font-size: 24px !important; font-weight: 700 !important; color: var(--text-primary) !important;
}

/* Expander */
.streamlit-expanderHeader {
    background: var(--surface) !important; border: 1px solid var(--border) !important;
    color: var(--text-primary) !important; border-radius: 10px !important; padding: 1rem !important; font-weight: 700 !important;
}
.streamlit-expanderHeader:hover { background: var(--surface-hover) !important; }
.streamlit-expanderContent {
    background: var(--surface) !important; border: 1px solid var(--border) !important; border-top: none !important;
    border-radius: 0 0 10px 10px !important; padding: 1.25rem !important;
}
...
h3 { font-size: 16px !important; font-weight: 700 !important; margin: 1rem 0 0.5rem 0 !important; }
...
/* Footer */
.footer-text {
    text-align: center; color: var(--text-tertiary); font-size: 12px; padding: 2rem 0 1rem 0;
    border-top: 1px solid var(--border-light);
}

/* Standardized trade card height */
.trade-card {
    min-height: 150px; height: 100%; display: flex; flex-direction: column; justify-content: space-between;
}
...
"""
    ...
    if action in ("BUY", "SHORT"):
        ...
        with col1:
            st.markdown(f"""
            <div class="trade-card" style="background: linear-gradient(145deg, #1e3a2c, #1a1a1a); border: 2px solid rgba(22, 199, 132, 0.4); border-radius: 16px; padding: 1.5rem;">
        ...
        with col2:
            st.markdown(f"""
            <div class="trade-card" style="background: linear-gradient(145deg, #3a1e1e, #1a1a1a); border: 2px solid rgba(234, 57, 67, 0.4); border-radius: 16px; padding: 1.5rem;">
        ...
        with col3:
            st.markdown(f"""
            <div class="trade-card" style="background: linear-gradient(145deg, #1e2a3a, #1a1a1a); border: 2px solid rgba(91, 127, 249, 0.4); border-radius: 16px; padding: 1.5rem;">
...
# TAB 2: Portfolio — amount only (no %)
with tabs[1]:
    ...
    if "last_signal" in st.session_state:
        ...
        if sig["action"] not in ("BUY", "SHORT"):
            ...
            st.markdown(f"""
            <div style="background: rgba(26, 26, 26, 0.8); border: 1px solid rgba(255, 255, 255, 0.1);
            ...
            col1, col2 = st.columns(2)
            with col1:
                max_amt = float(user_info['current_capital'])
                position_amount = st.number_input("Position Amount ($)", min_value=0.0, max_value=max_amt, value=min(1000.0, max_amt), step=100.0, format="%.2f")
            with col2:
                st.metric("Will Allocate", f"${position_amount:,.2f}")
            ...
            for i, tp_key in enumerate(['tp1', 'tp2', 'tp3'], 1):
                ...
                tp_data.append({"Level": f"TP{i}", "Price": f"${tp_price:.2f}", "Probability": f"{tp_prob}%", "Potential P&L": f"{pnl_pct:.2f}%"})
            if pd:
                st.dataframe(pd.DataFrame(tp_data), use_container_width=True, hide_index=True)

            st.markdown("---")
            st.markdown("""
            <div style="background: rgba(91, 127, 249, 0.1); border: 1px solid rgba(91, 127, 249, 0.3);
            ...
            if st.button("Add Trade to Portfolio", type="primary", use_container_width=True):
                try:
                    probs = sig["output"].get('probs') or {}
                    # Convert amount -> percent for DB storage if needed
                    position_pct = 0.0
                    cap = float(user_info['current_capital']) or 1.0
                    position_pct = max(0.0, min(100.0, (position_amount / cap) * 100.0))
                    ...
                    st.success(f"Trade #{trade_id} added to portfolio for ${position_amount:,.2f}")
                    st.rerun()
                except Exception as e:
                    st.error(f"Error: {e}")
    else:
        st.info("Run analysis and select a trade to add.")

# TAB 3: Active Trades (minimal list)
with tabs[2]:
...
with tabs[3]:
    ...
    perf = get_agent_performance(st.session_state.user['user_id'])
    if perf is None:
