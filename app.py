...
# Базовый движок сигналов (AI + правила + Universal)

from core.strategy import analyze_asset, analyze_asset_universal_v2
...
st.set_page_config(
page_title=“Arxora - трейд-ИИ (MVP)”,  # Заменил — на -
...
CUSTOM_PHRASES = {
“BUY”: [
“Точка входа: покупка в диапазоне {range_low}-{range_high}{unit_suffix}. По результатам AI-анализа выявлена ключевая область спроса в рамках текущего горизонта.”  # Заменил – на -
],
“SHORT”: [
“Точка входа: продажа (short) в диапазоне {range_low}-{range_high}{unit_suffix}. AI-анализ выявил точку продаж в рамках текущего горизонта.”  # Заменил – на -
],
“WAIT”: [
“Пока нет ясности с картиной на текущем горизонте, и я не спешу с решениями.”,
“Пока не стоит спешить - лучше дождаться более ясной картины. Вероятно, новости могут спровоцировать изменения на рынке.”,  # Заменил — на -
“Пока без позиции - жду более четкого сигнала. Новостной фон может сдвинуть рынок и повысить волатильность.”  # Заменил — на -
],
“CONTEXT”: {
“support”: [“Цена у уровня покупательской активности. Оптимальный вариант - открывать позицию на основе ордера, сгенерированного AI-анализом, рассчитанного на рост. Arxora учитывает множество факторов, которые трудно просчитать вручную, минимизируя эмоции и риски, и позволяя автоматически реагировать на быстро меняющиеся условия рынка. При этом важно контролировать риски: закрепление ниже этой зоны поддержки будет сигналом для пересмотра сценария. Торгуйте дисциплинированно. Строго соблюдайте уровни стоп-лосса.”],  # Заменил — на -
“resistance”: [“Цена подошла к сопротивлению, и вероятность коррекции здесь повышена. Оптимальный вариант - открывать позицию на основе ордера, сгенерированного AI-анализом, рассчитанного на падение. Такой подход обеспечивает максимально точный и своевременный вход в сделку. Arxora учитывает множество факторов, которые трудно просчитать вручную, минимизируя эмоции и риски, и позволяя автоматически реагировать на быстро меняющиеся условия рынка. При этом важно контролировать риски: закрепление выше зоны сопротивления будет сигналом для пересмотра сценария. Строго соблюдайте уровни стоп-лосса.”],  # Заменил — на -
“neutral”: [“Рынок пока в балансе - действую только по подтвержденному сигналу.”],  # Заменил — на -
“universal_entry_prime”: [“Основное время входа (10:00-11:30) EST/EDT. Стратегия основана на анализе 400+ успешных сделок. Рекомендация: держать до утра 9:31 AM или по достижении цели.”],
“universal_scalp”: [“Время скальпинга (14:00-15:30) EST/EDT. Быстрые позиции на 2-4 часа. Стратегия проверена на сотнях сделок.”],
“universal_exit”: [“Время выхода из overnight позиций (9:31 AM) EST/EDT. Не время для новых входов - лучше закрывать существующие позиции.”]
},
“STOPLINE”: [
“Стоп-лосс: {sl}. Потенциальный риск ~{risk_pct}% от входа. Уровень определен алгоритмами анализа волатильности как критический для защиты капитала.”  # Заменил ён на ен
],
...
entry = float(levels[“entry”]); sl = float(levels[“sl”])
return “—” if entry == 0 else f”{abs(entry - sl)/max(1e-9,abs(entry))*100.0:.1f}”  # Оставил — только здесь где нужен символ
...
“Горизонт”,
[“Краткосрок (1-5 дней)”, “Среднесрок (1-4 недели)”, “Долгосрок (1-6 месяцев)”],  # Заменил – на -
index=1,
key=“main_horizon”,
)
with col3:
...
“Universal Strategy”,
value=False,
help=“Стратегия на основе 400+ успешных сделок”,
key=“use_universal”
)
...
# Статус режима (AI/AI pseudo + Universal)
...
strategy_mode = “Universal” if use_universal else (“AI (pseudo)” if AI_PSEUDO else “AI”)
st.write(f”Mode: {strategy_mode} · Horizon: {hz_tag}”)
...
try:
# Выбор стратегии
if use_universal and “Кратко” in horizon:
out = analyze_asset_universal_v2(ticker=symbol_for_engine, horizon=horizon)
else:
out = analyze_asset(ticker=symbol_for_engine, horizon=horizon)
if use_universal:
st.warning(“Universal Strategy доступна только для краткосрочного горизонта. Используется основная стратегия.”)
...
```
    ...
    # Цвет заголовка в зависимости от стратегии
    header_color = "#8b5cf6" if use_universal else "#c57b0a"
    
    st.markdown(
        f"""
        <div style="background:{header_color}; padding:14px 16px; border-radius:16px; border:1px solid rgba(255,255,255,0.06); margin-bottom:10px;">
            ...
            {f"<div style='opacity:0.6; font-size:0.8rem; margin-top:2px;'>{strategy_mode} Strategy</div>" if use_universal else ""}
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if action in ("BUY", "SHORT"):
        ...
        # Дополнительная информация для Universal Strategy
        if use_universal and 'position_size' in out:
            st.markdown(
                f"<div style='margin-top:8px; padding:8px; background:#2d1b69; border-radius:8px;'>"
                f"<strong>Universal Strategy Info:</strong><br>"
                f"Рекомендуемый размер позиции: {out['position_size']} акций<br>"
                f"Тип инструмента: {out.get('instrument_type', 'N/A')}<br>"
                f"Торговая сессия: {out.get('time_analysis', {}).get('market_phase', 'N/A')}"
                f"</div>",
                unsafe_allow_html=True,
            )
    ...
    # Контекст в зависимости от стратегии
    if use_universal and 'time_analysis' in out:
        market_phase = out['time_analysis'].get('market_phase', '')
        if market_phase == "entry_prime":
            ctx_key = "universal_entry_prime"
        elif market_phase == "entry_scalp":
            ctx_key = "universal_scalp" 
        elif market_phase == "exit_morning":
            ctx_key = "universal_exit"
        else:
            ctx_key = "support" if action == "BUY" else ("resistance" if action == "SHORT" else "neutral")
    else:
        ctx_key = "support" if action == "BUY" else ("resistance" if action == "SHORT" else "neutral")
    
    if ctx_key in CUSTOM_PHRASES["CONTEXT"]:
    ...
    if out.get("alt"):
        st.markdown(
            f"<div style='margin-top:6px;'><b>Если пойдет против базового сценария:</b> {out['alt']}</div>",  # Заменил ё на е
            unsafe_allow_html=True,
        )

    # Показать note_html если есть (для Universal Strategy)
    if 'note_html' in out:
    ...
    # Показать подробную ошибку в режиме разработки
