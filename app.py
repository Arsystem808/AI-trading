...
CUSTOM_PHRASES = {
    ...
    "KIND_MSG": {
        # шаблоны для специфичных типов входа (entry_kind из strategy)
        "buy-stop": "Long • Buy Stop — вход выше текущей цены (пробой). Рассматриваем размещение стоп-заявки выше текущего уровня, с подтверждением ретеста/объёма.",
        "buy-limit": "Long • Buy Limit — ждём отката к уровню поддержки и размещаем лимитный ордер по выгодной цене.",
        "buy-now": "Long • Market price — текущая цена близка к зоне входа; возможен быстрый рынок (market). Контролируйте размер позиции и слежение за риском.",
        "sell-stop": "Short • Sell Stop — вход по стоп-ордеру ниже текущей цены (ожидается откат). Размещаем стоп-ордер и ждём подтверждающего отката.",
        "sell-limit": "Short • Sell Limit — размещаем лимитный ордер у сопротивления, смотрим на реакцию цены и объём.",
        "sell-now": "Short • Market price — текущая цена близка к варианту открытия; возможен быстрый рынок (market). Контролируйте риск.",
        # дополнительный особый текст, когда sell-stop и цена уже явно выше зоны сопротивления:
        "sell-stop_pulled_above": "Short • Sell Stop — цена уже второго уровня сопротивления; текущая рекомендация — не входить по рынку, а рассматривать установку стоп-ордера ниже для входа при откате/сигнале слабости сверху."
    },
...
if run and ticker:
    try:
        ...
        def render_plan_line(action, levels, ticker="", seed_extra=""):
            """
            Возвращает человекочитаемый план. Теперь учитываем entry_kind из out (если доступен)
            и даём более специфичные шаблоны для buy-stop / sell-stop / buy-limit / sell-limit / market.
            """
            # если стратегия вернула специфичный entry_kind / entry_label, используем его
            ek = out.get("entry_kind") or ""
            # special-case: если sell-stop и цена сейчас явно выше entry (т.е. уже протолкнули выше сопротивления),
            # даём отдельный текст "sell-stop_pulled_above"
            if ek == "sell-stop":
                # если текущая цена выше entry — это обычная ситуация для sell-stop (вход ниже текущей),
                # но если у нас есть контекст, например цена значительно выше последнего R1/пивота — можно указать особый текст.
                # Для простоты — если last_price > levels['entry'] + несколько шагов (тут 0) — берем special form.
                if float(last_price) > float(levels["entry"]) + 1e-9:
                    return CUSTOM_PHRASES["KIND_MSG"].get("sell-stop_pulled_above")
            # выбор по entry_kind
            if ek and (ek in CUSTOM_PHRASES["KIND_MSG"]):
                return CUSTOM_PHRASES["KIND_MSG"][ek]

            # fallback: старый общий шаблон (для BUY/SHORT/WAIT)
