...
if st.session_state.get('show_arxora', False):
    st.markdown(
        ...
        <div style="background-color: #000000; color: #ffffff; padding: 15px; border-radius: 10px; margin-top: 10px;">
            ...
            делая торговлю проще, эффективнее и разумнее. Ключевые особенности платформы: AI override: Интеллектуальная функция, которая в режиме реального времени может корректировать или отменять ранее сгенерированные сигналы и ордера, учитывая новые данные и экстремальные рыночные ситуации, что повышает безопасность и оптимальность торговых решений.
